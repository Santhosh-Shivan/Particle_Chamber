...
class YoloDataGenerator(ContinuousGenerator):
    """implement Dataset here"""

    def __init__(self, input_size, num_class, **kwargs):
    ...
    def __getitem__(self, idx):
        ...
        images, bboxes = super().__getitem__(idx)

        num = 0

        for num in range(self.batch_size):

            image, bbox = utils.image_preprocess(
                np.copy(images[num]),
            ...
            (
                ...
                mbboxes,
                lbboxes,
            ) = self.preprocess_true_boxes(bbox)
